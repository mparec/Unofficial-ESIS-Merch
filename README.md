# Unofficial ESIS Merch
```
:::::::::: :::::::: ::::::::::: :::::::: 
:+:       :+:    :+:    :+:    :+:    :+:
+:+       +:+           +:+    +:+       
+#++:++#  +#++:++#++    +#+    +#++:++#++
+#+              +#+    +#+           +#+
#+#       #+#    #+#    #+#    #+#    #+#
########## ######## ########### ######## 
```
## Descripción
Este programa implementado en C++ simula la venta de artículos exclusivos de la <b>ESIS - UNJBG</b>.
## Estructura del proyecto
  <b>`main.cpp`</b> : Archivo del script principal.<br>
  <b>`cabecera.h`</b> : Declaración de funciones.<br>
  <b>`cabecera.cpp`</b> : Definición de funciones.<br>
>>>>>>> origin/carles

> [!NOTE]
> Usa las teclas numéricas del 0 al 4 para interactuar con las opciones del menú principal.

> [!WARNING]
> Respeta las entradas solicitadas por el programa.

## Autores
<ul>
  <li>Matías Pare</li>
  <li>Carles Vilca</li>
  <li>Marco Valeriano</li>
</ul>